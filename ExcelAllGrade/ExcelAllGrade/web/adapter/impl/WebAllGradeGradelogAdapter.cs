...
{
    /// <inheritdoc />
    public class WebAllGradeGradelogAdapter : IWebAllGradeGradelogAdapter
    {
        /// <inheritdoc />
        ...
        {
            string baseUri = Constants.SERVER_HOST + ":" + Constants.PORT + Constants.PATH_GET_GRADELOGS;

            string parameterString = "?";
            parameterString += Constants.PARAM_CLASS + "=" + classToken + "&";
            parameterString += Constants.PARAM_SUBJECT + "=" + subjectToken + "&";
                ...
                using (WebResponse webResponse = webRequest.GetResponse())
                {
                    using (Stream responseStream = webResponse.GetResponseStream())
                    {
                        using (StreamReader reader = new StreamReader(responseStream))
                        {
                            ...
                            return JsonBuilder.deserializeGradelogList(responseString);
                        }
                    }
                }
            }
